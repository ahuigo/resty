...
func TestPostParams(t *testing.T) {
	...
	resp, err := resty.New().R().SetResult(&data).
		SetQueryParams(MapString{"name": "Alex"}).Post(
		ts.URL + "/post",
	)
	if err != nil {
		t.Fatal(err)
...
func TestPostFormData(t *testing.T) {
	...
	if !strings.Contains(data.Data, "form-data; name=\"name\"\r\n\r\nAlex\r\n") {
...
func TestRawString(t *testing.T) {
	...
	if data.(map[string]interface{})["data"].(string) != rawText {
		t.Error("invalid response body data:", resp.String())
	}
}
