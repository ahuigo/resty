...
func TestSendCookie(t *testing.T) {
	...
	resp, err := resty.New().R().SetResult(&data).SetHeader("Cookie", "token=1234").Get(ts.URL + "/cookies")
	if err != nil {
		panic(err)
	}
	if !strings.Contains(resp.String(), `{"token":"1234"}`) {
...
func TestSessionCookie(t *testing.T) {
	...
	// 1. set cookie1
	session.SetCookie(cookie1).Get(ts.URL + "/cookies/count")

	// 2. set cookie2 and get all cookies
	resp, err := session.SetCookie(cookie2).SetResult(&result).Get(ts.URL + "/cookies/count")
	if err != nil {
		t.Fatal(err)
	}
	...
	if result.Name1 != "value1" || result.Name2 != "value2" {
		t.Fatalf("Failed to send valid cookie(%+v)", resp.String())
	}
...
func TestResponseCookie(t *testing.T) {
	...
	session := resty.New().R()
	resp, err := session.Get(ts.URL + "/cookies/count")
	if err != nil {
		t.Fatal(err)
	}
